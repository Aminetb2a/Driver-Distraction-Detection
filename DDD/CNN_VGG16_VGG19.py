...
def save_model(model):
    ...
    if not os.path.isdir('cache'):
        os.mkdir('cache')
    open(os.path.join('cache', 'architecture_vgg166666.json'), 'w').write(json_string)
    model.save_weights(os.path.join('cache', 'model_weights_vggg16666.h5'), overwrite=True)
...
def VGG_16_Model(img_width, img_height):
    ...
    # compile the model 
    AdamO=Adam(lr=0.001, beta_1=0.9, beta_2=0.999, epsilon=1e-08, decay=0.0)
...
def VGG_19_Model():
    ...
    AdamO=Adam(lr=0.1, beta_1=0.9, beta_2=0.999,epsilon=1e-08,decay=0.0)
    model.compile(optimizer=AdamO, loss='categorical_crossentropy')
...
reducelr=ReduceLROnPlateau(monitor='val_loss',factor=0.1,patience=5,min_lr=0.001)
...
hist = model.fit_generator(generator=train_generator, steps_per_epoch=nb_train_samples/batch_size, epochs=nb_epoch, 
                     callbacks=[reducelr],validation_data=val_generator, validation_steps=nb_validation_samples/batch_size)
...
with open("model_vgg16_3.json", "w") as json_file:
    json_file.write(model_json)

# serialize weights to HDF5
model.save_weights("model_vgg16_3.h5")
